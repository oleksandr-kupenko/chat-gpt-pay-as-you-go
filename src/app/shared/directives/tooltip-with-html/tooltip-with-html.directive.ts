...
export abstract class _MatTooltipBase<T extends _TooltipComponentBase> implements OnDestroy, AfterViewInit {
  ...
  private _customOffset!: number;
  private _message = '';
  private _isListenClick!: boolean;
  ...
  @Input() htmlTooltipCustomOffset = 0;
  get customOffset(): number {
    return this._customOffset;
  }

  /** Allows the user to define the position of the tooltip relative to the parent element */
  @Input('htmlTooltipPosition')
   ...
   * instead of outside the element bounding box.
   */
  @Input('htmlTooltipPositionAtOrigin')
   ...
   */
  @Input('htmlTooltipTouchGestures') touchGestures: TooltipTouchGestures = 'auto';
  ...
  @Input('htmlTooltipIsListenClick')
  get isListenLinkClicks() {
    return this._isListenClick;
  }

  set isListenLinkClicks(value: boolean) {
    this._isListenClick = value;
  }

  /** Classes to be passed to the tooltip. Supports the same syntax as `ngClass`. */
  @Input('htmlTooltipClass')
  get tooltipClass() {
  ...
  @Output() clickedLinkId = new EventEmitter<string>();
  ...
  private customSubs = new Subscription();
  ...
  show(delay: number | undefined = this.showDelay, origin?: {x: number; y: number}): void {
    ...
    const instance = (this._tooltipInstance = overlayRef.attach<any>(this._portal).instance);
    instance.isListenLinkClicks = this.isListenLinkClicks;

    if (this.isListenLinkClicks) {
      this.customSubs.add(
        instance.linkClick.subscribe((data: string) => {
          this.clickedLinkId.emit(data);
        }),
      );
    }
  ...
  private _setTooltipClass(tooltipClass: string | string[] | Set<string> | {[key: string]: any}) {
    if (this._tooltipInstance) {
      ...
      this._tooltipInstance._tooltip?.nativeElement?.classList.add(tooltipClass as string);
      this._tooltipInstance._markForCheck();
    }
  }
...
@Directive({
  ...
  exportAs: 'htmlTooltip',
  host: {
    class: 'mat-mdc-tooltip-trigger',
    '[class.mat-mdc-tooltip-disabled]': 'disabled',
  },
})
export class TooltipWithHtmlDirective extends _MatTooltipBase<TooltipComponent> {
  ...
  protected override _addOffset(position: ConnectedPosition): ConnectedPosition {
    const offset = UNBOUNDED_ANCHOR_GAP + this.customOffset;
    const isLtr = !this._dir || this._dir.value == 'ltr';
...
@Component({
  ...
  template: `<div
    #tooltip
    class="mdc-tooltip mdc-tooltip--shown mat-mdc-tooltip custom-tooltip"
    (animationend)="_handleAnimationEnd($event)"
    [class.mdc-tooltip--multiline]="_isMultiline"
  >
    <div #tooltipContentRef class="mdc-tooltip__surface mdc-tooltip__surface-animation"></div>
  </div>`,
  styleUrls: ['./tooltip-with-html.component.scss'],
...
export class TooltipComponent extends _TooltipComponentBase implements AfterViewInit {
  @Input() isListenLinkClicks?: boolean;
  ...
  ngAfterViewInit() {
    if (this.isListenLinkClicks) {
      setTimeout(() => {
        this.addClickListenerForLinks(); //sends link id to the top when clicked
      }, 200);
    }
  }

  private addClickListenerForLinks() {
    const links = this.tooltipContent.nativeElement.querySelectorAll('a');
    links.forEach((link: HTMLAnchorElement) => {
      this.renderer.listen(link, 'click', () => {
        this.linkClick.emit(link.id);
      });
    });
  }
  ...
  protected override _onShow(): void {
    ...
    this.tooltipContent.nativeElement.innerHTML = this.message;
    this._markForCheck();
  }
